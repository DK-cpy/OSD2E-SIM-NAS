...
class GenTrainer():
    ...
    def pso_optimize(self, population, fid_stat):
        ...
        def fitness_function(x):
            ...
            return fid_value - is_value  # 多目标优化（最小化FID，最大化IS）
    ...
    def my_search_evolv2(self, parents, fid_stat, num):
        ...
        for i, parent in enumerate(parents):
            if not isinstance(parent, np.ndarray) or parent.shape != (3, 7):
                print(
                    f"Element {i} in parents has an unexpected shape: {parent.shape if isinstance(parent, np.ndarray) else type(parent)}")
                raise ValueError("All elements in parents should be 3x7 numpy arrays.")
        # 将列表转换为 numpy 数组
        parents_array = np.array(parents)
        offsprings = self.gen_offspring(parents_array)  # 传入数组而非列表
        genotypes = np.concatenate((parents_array, offsprings), axis=0)
        is_values, fid_values, params = np.zeros(len(genotypes)), np.zeros(
            len(genotypes)), np.zeros(len(genotypes))
        # 设定默认值
        keep_N = len(offsprings)
        # 假设 selected_N 的默认值为 10，你可以根据实际情况修改
        selected_N = 10
        for idx, genotype_G in enumerate(tqdm(genotypes)):
            # 确保 genotype_G 中的元素是整数类型
            genotype_G = genotype_G.astype(int)
    ...
    def validate(self, genotype_G, fid_stat):
        ...
        try:
            ...
            if not np.issubdtype(genotype.dtype, np.integer):
                genotype = genotype.astype(int)  # 确保基因型元素为整数类型
        ...
        with torch.no_grad():
            for iter_idx in tqdm(range(eval_iter), desc='sample images'):
                ...
                gen_imgs = (gen_net(z, genotype) + 1) / 2  # 使用转换后的基因型
    ...
    def uniform_crossover_mutation(self, alphas_a, alphas_b):
        ...
        # Mutation operation
        num_mute = random.randint(1, self.args.mute_max_num if hasattr(self.args, 'mute_max_num') else 1)
