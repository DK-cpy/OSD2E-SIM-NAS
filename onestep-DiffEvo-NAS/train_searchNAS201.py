...
parser.add_argument('--config_path', type=str, default='./configs/imageNet-16.config', help='The config path.')
...
def validation(model, valid_queue, criterion, gen):
    ...
    for i in range(len(population.get_population())):
        logging.info(
            "[{} Generation] {}/{} finished with validation loss: {}, prec1: {}, prec5: {}".format(gen, i + 1, len(
                population.get_population()),
                                                                                                   population.get_population()[
                                                                                                       i].objs.avg,
                                                                                                   population.get_population()[
